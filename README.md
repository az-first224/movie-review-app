...
- `id`: UUID、主キー
- `title`: 作品タイトル
- `release_date`: 作品の年月日（任意）
- `release_year`: 旧データの年度表示に使用する互換カラム
- `director`: 監督名
- `tagline`: キャッチコピー
- `cast_names`: 出演者
- `rating`: 0.0〜5.0の評価点
...
3. 入力フォームに作品情報を入力します。
4. 作品日付は年月日まで入力できます（任意）。
5. 星評価はスライダーまたは数値入力で、0.0から5.0まで0.1単位で指定できます。
6. カテゴリータグは `映画`、`漫画`、`ドラマ`、`アニメ`、`ゲーム`、`スポーツ`、`小説`、`音楽` から1つ選択します。
7. ジャンルは複数選択できます。
8. `登録する` を押すとSupabaseへ保存されます。
...
- レビュー一覧を1ページ10件で表示
- 更新日時、登録日時の新しい順で表示
- 作品タイトル、作品日付、監督名、出演者、ジャンル、カテゴリー、コメント本文で検索
- 星評価で絞り込み
- ジャンルで絞り込み
- カテゴリーで絞り込み
- 登録日時と更新日時の表示
